#!/usr/bin/env python3
"""付印线·assemble(工序 s6b-assemble-binding):把 vendor 里的能力接到工作区,原地跑一遍。

用法:
  print_assemble.py --workspace X [--volume V]
...
def main() -> int:
    ...
    if cfg["missing"]:
        print(json.dumps({"step": "s6b-assemble-binding", "status": "unbound", "missing": cfg["missing"],
                          "why": "付印所需的册级绑定缺项。封面/封底/键名是使用方的事实,不猜。"},
                         ensure_ascii=False, indent=1))
        return 1
    m = P.bind_assemble(CHAIN, cfg["key"], cfg["cover"], cfg["back"], cfg["pdfName"])
    (CHAIN.workspace / "output" / "print" / "standard-pdf").mkdir(parents=True, exist_ok=True)
    try:
        m.main()
        ...
        if isinstance(exc.code, int) and exc.code == 0:
            ...
            print(json.dumps({"step": "s6b-assemble-binding", "status": "failed",
    ...
    print(json.dumps({"step": "s6b-assemble-binding", "status": status, "report": str(report_path)},
                     ensure_ascii=False, indent=1))
