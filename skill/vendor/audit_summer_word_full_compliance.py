...
class DocxAudit:
    ...
    def audit_path_and_name(self) -> None:
        # 末尾可选的「范围」段。
        #
        # ★首版写作 (-[^-]+)? —— 一段、且**不含连字符**。那不是想出来的规则,
        #   是被当时唯一那一个文件名塑造的:旧册叫「第A10至A14讲」,用「至」连接。
        #   而正式库里既有的分册文件一直用连字符(第A01-A09讲 / 第A10-A19讲),
        #   两边从来不一致,只是流程只出过「至」那一种,所以没撞上。
        #   2026-08-20 出第A01-A20讲时撞上了。
        #
        # 改法是**更具体**而不是更宽松:范围段必须长得像一个范围
        # (第A01-A20讲 / 第A10至A14讲),而不是放任任意字符。
        # 原来的单段形式保留——「上」「下」这类分册词仍然合法。
        expected = re.compile(
            r"^2026-暑假班-(七年级|八年级)-(上册|全一册)-(语文|英语|物理|化学)-(学生版|教师版)-(讲义|习题册)"
            r"(-第[A-Z]?\d{1,2}[-至][A-Z]?\d{1,2}讲|-[^-]+)?\.docx$"
        )
        name = self.path.name
